...
def loadBothConstraints(stria, strib, res):
    ...
    per_a       = np.percentile(mata, 99.99)   # for population its 99.0; for single cell its 99.99
    ...
    per_b       = np.percentile(matb, 99.99)   # for population its 99.0; for single cell its 99.99
...
class GSE130711Module(pl.LightningDataModule):
    def __init__(self,
        ...
        self.cellNo = cell_No
        self.dirname = str(root) + "/DataFull" + "/unNormalization_"+self.cellLine+"_cell"+str(self.cellNo)+"_"+str(self.res)+"_"+deg+"_"+str(sigma_0)
