...
class GaussianDiffusion(nn.Module):
    ...
    def super_resolution(self, x_in, continous=False):   # here x_in is nosisy data in the dataloader
        return self.p_sample_loop(x_in, continous)

    @torch.no_grad()  # only the interpolate cannot find out where it will be used
    def interpolate(self, x1, x2, t = None, lam = 0.5):  # should be modified, but we will not use
        ...
        self_cond = None  # for interpolate is unconditional
        for i in tqdm(reversed(range(0, t)), desc = 'interpolation valuate time step', total = t):
            img, _ = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), self_cond)

        return img

    # below is the forward process to valuate the current from noise, function is used in p_losses()
    def q_sample(self, x_start, t, noise=None):  # here noise is gaussion  noise which is used for forward-adding noising valuate
    ...
    def p_losses(self, x_in, t = None, noise = None):   # here the x_in the whole dataloader in datasets
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=x_end.device).long()  # t is 1D dimension data same as the batch_size

        noise = default(noise, lambda: torch.randn_like(x_end))  # here is the gaussion noise

        # noise valuate
        x = self.q_sample(x_start = x_end, t = t, noise = noise)  # here get x from gaussion noise sampling
