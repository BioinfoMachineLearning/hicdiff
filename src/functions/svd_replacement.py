...
#a memory inefficient implementation for any general degradation H  一种内存效率低下但普适的退化矩阵
...
#Super Resolution   超分变率
class SuperResolution(H_functions):
    def __init__(self, channels, img_dim, ratio, device): #ratio = 2, 4 or 8
...
#Walsh-Hadamard Compressive Sensing   压缩感知，压缩采样/稀疏采样
...
#Convolution-based super-resolution  基于卷积的超分辨率
...
#Anisotropic Deblurring  各项异性去模糊
class Deblurring2D(H_functions):
    ...
    def singulars(self):
        return self._singulars.repeat(1, self.channels).reshape(-1)  # here the second
