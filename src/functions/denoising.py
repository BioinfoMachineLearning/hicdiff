...
def compute_alpha(beta, t):  #this is Cumulative product of alphas
    beta = torch.cat([torch.zeros(1).to(beta.device), beta], dim=0)
    a = (1 - beta).cumprod(dim=0).index_select(0, t + 1).view(-1, 1, 1, 1)
    return a

def efficient_generalized_steps(x, seq, model, b, H_funcs, y_0, sigma_0, etaB, etaA, etaC, cls_fn=None, classes=None, device = None):
    with torch.no_grad():
        #setup vectors used in the algorithm
        singulars = H_funcs.singulars()  # si
        Sigma = torch.zeros(x.shape[1]*x.shape[2]*x.shape[3], device=x.device)
        Sigma[:singulars.shape[0]] = singulars  # this is the si vector with the shape as input data
        U_t_y = H_funcs.Ut(y_0)
        Sig_inv_U_t_y = U_t_y / singulars[:U_t_y.shape[-1]]

        #initialize x_T as given in the paper
        largest_alphas = compute_alpha(b, (torch.ones(x.size(0)) * seq[-1]).to(x.device).long())  #this is Cumulative product of alphas
        largest_sigmas = (1 - largest_alphas).sqrt() / largest_alphas.sqrt()
        large_singulars_index = torch.where(singulars * largest_sigmas[0, 0, 0, 0] > sigma_0)

        # to compute the sigma_y/singlar_values, note: sigma_y == sigma_0
        ...
        # implement p(x_T | x_0, y) as given in the paper in the spectral space
        ...
        #setup iteration variables in pixels' space
        x = H_funcs.V(init_y.view(x.size(0), -1)).view(*x.size())  # converting to the pixels' space from spectral space
        n = x.size(0)
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()  # from predict noise to the x0 at time t, et is the predicted noise at time t in pixels' space

            #variational inference conditioned on y
            sigma = (1 - at).sqrt()[0, 0, 0, 0] / at.sqrt()[0, 0, 0, 0]
            sigma_next = (1 - at_next).sqrt()[0, 0, 0, 0] / at_next.sqrt()[0, 0, 0, 0]   # here sigma_next = sigma_t in paper, differ from the Sigma, singular value(si) vector with the same shape as input data
            xt_mod = xt / at.sqrt()[0, 0, 0, 0]
            V_t_x = H_funcs.Vt(xt_mod)  # convert to spectral space
            SVt_x = (V_t_x * Sigma)[:, :U_t_y.shape[1]]
            V_t_x0 = H_funcs.Vt(x0_t)   # convert to spectral space
            ...
            std_nextC = sigma_next * etaC  # etaC == etaA in these two situations
            ...
            #noisier than y (before)  # the third situation in paper's equation
            ...
            xt_mod_next = H_funcs.V(Vt_xt_mod_next)  # get missing value in pixels space at time t
