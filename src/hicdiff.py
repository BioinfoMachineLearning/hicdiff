...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        register_buffer = lambda name, val: self.register_buffer(name, val.to(torch.float32)) # note this parameters wrapped by register_function will not change or be trained by the model
    ...
    def q_posterior(self, x_start, x_t, t):  # this is somethng that for equal 6 which will be used in p_mean_variance() function
        posterior_mean = (   # the "t" here is the same shape as coef1/coef2, t here also contains all the same int values
    ...
    def p_sample(self, x, t: int, x_self_cond = None):
        ...
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)  # this to make sure all the element in the batched_times are equal.
    ...
    def p_sample_loop(self, shape, return_all_timesteps = False):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            self_cond = x_start if self.self_condition else None   # self_cond = None always for the first iteration
            img, x_start = self.p_sample(img, t, self_cond)
            imgs.append(img)

        ret = img if not return_all_timesteps else torch.stack(imgs, dim = 1)  # stack with channels axis
    ...
    @torch.no_grad()   # below sample() function will introduce ddim_sample() and p_sample_loop() this two function
    def sample(self, x, return_all_timesteps = False):
    ...
    @torch.no_grad()  # only the interpolate cannot find out where it will be used
    ...
    # below is the forward process to sample the current from noise, function is used in p_losses()
    def q_sample(self, x_start, t, noise=None):  # here noise is gaussion  noise which is used for forward-adding noising sample
    ...
    def p_losses(self, x_start, t, noise = None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))  # here is the gaussion noise

        # noise sample

        x = self.q_sample(x_start = x_start, t = t, noise = noise)  # here get x from gaussion noise sampling
    ...
    def forward(self, img, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()  # t is 1D dimension data same as the batch_size
