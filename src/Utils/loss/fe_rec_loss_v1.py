...
from src.model.DAE_model import DAE

class FeatureReconstructionLoss(nn.Module):
    def __init__(self, layers = [0,1,2,3,4], file = "../pretrained", device = None):
        super(FeatureReconstructionLoss, self).__init__()
        loss_net = DAE()

        file_path = glob.glob(file)[0]
        ...
        for layer in layers:
            ...
            sub_net = nn.Sequential(*(list_of_layers + final_layer)).float().eval().cuda(device=self.device)
    ...
    def forward(self, prediction, target):
        ...
        feature_loss = torch.tensor([0.0]).float().cuda(device=self.device)
