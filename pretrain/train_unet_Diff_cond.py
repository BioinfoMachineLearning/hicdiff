...
class HiCDiff:
    def __init__(self, epoch = 400, timestep = 1000, cell_Line = 'Human',  cellNo = 1, res = 40000, batch_size = 64, piece_s = 64, sigma = 0.1, deg='deno'):
        ...
        wandb.run.name = f'Unet_Diff_conditional_L2_linear cell_{cellNo}'
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            with torch.no_grad():
                ...
                if now_ssim > best_ssim:
                    ...
                    '''
                    best_ckpt_file = f'bestg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_unet_cond_l2_lin.pytorch'
                    torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, best_ckpt_file))
                wandb.log({"Epoch": epoch, 'train/loss': train_loss, 'valid/loss': valid_loss, 'psnr': batch_psnr})

        final_ckpt_g = f'finalg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_unet_cond_l2_lin.pytorch'
        torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, final_ckpt_g))
        '''

if __name__ == "__main__":
    train_model = HiCDiff(epoch = 1, batch_size = 64, cellNo = 1, sigma = 0.1, deg = 'deno')  # default is 400
