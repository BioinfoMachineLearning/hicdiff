...
from src.hicdiff_condition import Unet, GaussianDiffusion  # baseline models' modules
...
class HiCDiff:
    def __init__(self, epoch = 400, timestep = 1000, cell_Line = 'Human',  cellNo = 1, res = 40000, batch_size = 64, piece_s = 64, sigma = 0.1, deg='deno', schedular = 'linear'):
        ...
        wandb.run.name = f'transfer learning: Unet_Diff_condition_L2_{self.schedule[:3]}_sigma:{self.sigma} cell_{cellNo}'
    ...
    def fit_model(self):
        ...
        path = os.path.join(self.out_dir, 'bestg_40000_c64_s64_Human1_unet_cond_l2_sig.pytorch')
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for data, target, _, info in train_bar:
            ...
            batch_id = 0
            with torch.no_grad():
                for data, target, _, info in valid_bar:  # data is pure image without noise
                    batch_size = data.shape[0]
                ...
                if now_ssim > best_ssim:
                    best_ssim = now_ssim
                    print(f'Now, Best loss is {best_ssim:.6f}')
                    best_ckpt_file = f'bestg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_{self.sigma}_unet_cond_l2_{self.schedule[:3]}_trans.pytorch'
                    torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, best_ckpt_file))
                wandb.log({"Epoch": epoch, 'train/loss': train_loss, 'valid/loss': valid_loss, 'valid/ssim': batch_ssim,
                     'valid/psnr': batch_psnr})

        final_ckpt_g = f'finalg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_{self.sigma}_unet_cond_l2_{self.schedule[:3]}_trans.pytorch'
        torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, final_ckpt_g))

if __name__ == "__main__":
    train_model = HiCDiff(epoch = 400, batch_size = 64, cellNo = 2, sigma = 0.1, deg = 'deno', schedular='sigmoid')  # default is 400
