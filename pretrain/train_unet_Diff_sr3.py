...
from src.hicdiff_sr3 import Unet, GaussianDiffusion  # baseline models' modules
...
class HiCDiff:
    def __init__(self, epoch = 400, timestep = 2000, cell_Line = 'Human',  cellNo = 1, res = 40000, batch_size = 64, piece_s = 64, sigma = 0.1, deg='deno'):
        ...
        wandb.run.name = f'Unet_Diff_cond_sr3_linear cell_{cellNo}'
    ...
    def fit_model(self):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            with torch.no_grad():
                ...
                if now_loss < best_loss:
                    ...
                    best_ckpt_file = f'bestg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_unet_cond_sr3_lin.pytorch'
        ...
        final_ckpt_g = f'finalg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_unet_cond_sr3_lin.pytorch'
        torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, final_ckpt_g))


if __name__ == "__main__":
    train_model = HiCDiff(epoch = 400, batch_size = 64, cellNo = 2, sigma = 0.1, deg = 'deno')  # default is 400
