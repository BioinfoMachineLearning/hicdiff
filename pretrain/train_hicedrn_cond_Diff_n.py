...
class HiCedrn_Diff:
    def __init__(self, epoch = 500, timestep = 1000, cell_Line = 'Human',  cellNo = 1, res = 40000, batch_size = 64, piece_s = 64, sigma = 0.1, deg='deno', schedular = 'linear'):
        ...
        wandb.run.name = f'hicedrn_Diff_condition_L2_{self.schedule[:3]}_sigma:{self.sigma} cell_{cellNo}'
    ...
    def fit_model(self):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            with torch.no_grad():
                ...
                if now_ssim > best_ssim:
                    ...
                    best_ckpt_file = f'bestg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_{self.sigma}_HiCedrn_cond_l2_{self.schedule[:3]}.pytorch'
        ...
        final_ckpt_file = f'finalg_{self.res}_c{self.chunk}_s{self.chunk}_{self.cell_Line}{self.cellNo}_{self.sigma}_HiCedrn_cond_l2_{self.schedule[:3]}.pytorch'
        torch.save(self.diffusion.state_dict(), os.path.join(self.out_dir, final_ckpt_file))

if __name__ == "__main__":
    train_model = HiCedrn_Diff(epoch = 400, batch_size = 64, cellNo = 2, sigma = 0.1, deg = 'deno', schedular='linear')
